...
public class SixenseCam : MonoBehaviour {
	...
	void Update() {
		if (sixCtlMain.firstRun) {
			...
			// left/right, up/down
			Vector3 dirVectorMain = Vector3.zero;
			if (verticalRotation) {
				dirVectorMain = transform.localRotation * (new Vector3(sixCtlMain.joystickVal.x, sixCtlMain.joystickVal.y, 0f) * joystickScale);
			} else {
				dirVectorMain = transform.localRotation * (new Vector3(sixCtlMain.joystickVal.x, 0f, 0f) * joystickScale);
			}
